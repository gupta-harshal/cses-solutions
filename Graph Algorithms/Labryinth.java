...
public class Labryinth {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            String line = jio.nextLine();
            for (int j = 0; j < m; j++) {
                st[i][j] = line.charAt(j);
                if (st[i][j] == 'A') {
                    startX = i;
                    startY = j;
                }
        ...
        if (bfs(startX, startY)) {
            jio.println("YES");
            jio.println(ans.length());
            jio.println(ans.toString());
        } else {
            jio.println("NO");
        }
    }
    ...
    static boolean bfs(int startX, int startY) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (x >= 0 && x < n && y >= 0 && y < m && st[x][y] != '#' && !vis[x][y]) {
                    vis[x][y] = true;
                    parent[x][y] = r * m + c;
                    dir[x][y] = i;
                    q.add(new int[]{x, y});
