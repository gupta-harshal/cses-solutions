...
public class TowerOfHanoi extends MyTemplate.Calc {
    static MyTemplate.FastIO jio;

    public static void main(String[] args) throws IOException {
        jio = new MyTemplate().new FastIO();
        int n = jio.nextInt();
        int totalMoves = (1 << n) - 1;
        jio.println(totalMoves);
        solve(n, 1, 2, 3);
        
        jio.flush();
    }

    public static void solve(int n, int src, int aux, int dest) throws IOException {
        if (n == 1) {
            jio.print(src);
            jio.print(' ');
            jio.println(dest);
